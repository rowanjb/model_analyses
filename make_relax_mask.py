...
def relax_mask(example_bin, shape, side_cells, side_max_M, bottom_cells, bottom_max_M, bin_name):
    ...
    The scaling of the cells is halved every dx, e.g., 
    ...
    M[:] = 0 # Most (interior) M_rbc cells should be 0
    ...
    fig, ax = plt.subplots()
    cs = ax.pcolormesh(X, Y, M[20,:,:])
    cbar = fig.colorbar(cs)
    plt.savefig('relax_mask.png')

if __name__ == "__main__":
    example_bin = '../MITgcm/so_plumes/binaries/V.motionless.150x150.bin'
    shape = (50,150,150)
    side_cells = 5
    side_max_M = 0.8
    bottom_cells = 5 
    bottom_max_M = 0.8
    bin_name = 'relax_mask_incl_bottom_gentle_grad.150x150.bin'
    relax_mask(example_bin, shape, side_cells, side_max_M, bottom_cells, bottom_max_M, bin_name)
